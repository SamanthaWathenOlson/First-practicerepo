...
def test_create_customer_operational_error_caught():
    try:
        customer_dao.delete_from_customer_table_by_id(-1)
        assert False
...
def test_create_customer_with_malformed_id():
    ...
    result= customer_dao.insert_into_customer_table(customer)
    assert result.customer_id != 0


def test_delete_customer_record_success():
    result = customer_dao.delete_from_customer_table_by_id(-1)  # will have to 1 to positive number to delete a record
    assert result
